...
- 可破坏物返回 `onHit(hit, api)`, 不使用敌人的 `damage`; `hit` 含射线命中信息及 `direction/normal/damage/team`, 返回 `{ bulletmark: false }` 可取消本次弹痕. 物体需配置碰撞以进入射击目标, 参考 `street-lamp.js`; 碎片通过独立临时特效生成.
...
- 总图四角各 100×76 米, 中间留 20 米十字连接带与中心广场; 西南为现有海滨车站, 西北预留神社公园, 东北商店街, 东南渔港. `scene-kamakura.js` 的 `regionPlan` 记录分区; `district-ground.js` 只生成其余三区和连接带的平地及外围挡墙, 顶面 Y=2.4, 通过站区北/东侧开口相连. 原站区坐标不变, 12 级楼梯连接低处车站与住宅台地; `neighborhood` 配置尺寸/碰撞, `station-neighborhood.js` 管台地, `coastal-beach.js` 管沙滩. 不生成外围山坡和隧道.
